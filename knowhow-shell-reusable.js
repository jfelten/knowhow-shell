...
var createTerminal = function(callback) {
	var shell="bash";
	var args = [];
	...
	promptCommand = {
		"command": "PS1="+knowhowShellPrompt+"; PROMPT_COMMAND="+knowhowShellPromptCommand+"\r"
	}
	promptCommand.callback=callback
	scriptRuntime.currentCommand = promptCommand;
	...
	term.on('data', function(data) {
		...
		if (data) {
		 ...
		 if (!scriptRuntime.currentCommand.noLog) {
		  ...
		  if (scriptRuntime.currentCommand.output.match(retCodeRE)) {
		  	scriptRuntime.progress=scriptRuntime.currentStep*scriptRuntime.progressStepLength;
		  	status = 'completed: '+scriptRuntime.currentCommand.command;
		  	var id = "none"
		  	if (scriptRuntime.job) {
		  		id = scriptRuntime.job.id;
		  	}
		  	eventEmitter.emit('job-update',{id: id, status: status, progress: scriptRuntime.progress});
...
var destroyTerminal = function() {
	term.end();
}

exports.createTerminal = createTerminal;
exports.destroyTerminal = destroyTerminal;
...
var executeJob = function(job, callback) {
	knowhowInterpreter.setEnv(job, function(err) {
		...
		envCommand = {command:""};//"noLog": true};
		for (env in job.script.env) {
			if (env && env != '' && job.script.env[env] && job.script.env[env] !='') {
				envCommand.command+=env+'=\"'+job.script.env[env]+"\"";
				if (envCommand.command.slice(-1)!=';') {
					envCommand.command+='; ';
				}
			}
		}
		envCommand.command+="working_dir="+job.working_dir+";";
		newCommands1.push(envCommand);
		if (!job.workingDir) {
			job.workingDir="./"
		}
		newCommands1.push({
			"command": "cd "+job.workingDir,
			//"noLog": true
		});
		for (index = 0; index < job.script.commands.length; index++) {
			newCommands1.push(job.script.commands[index]);
		}
...
KnowhowShell.prototype.createTerminal = createTerminal;
KnowhowShell.prototype.destroyTerminal = destroyTerminal;
